...
console.log(buyukSayiBulma(sayilar));

const kucukSayiBulma = (dizi) => {
  let enkucuk = dizi[0];
  for (let i = 0; i < dizi.length; i++) {
    if (dizi[i] < enkucuk) {
      enkucuk = dizi[i];
...
console.log(kucukSayiBulma(sayilar));
...
ucebolunenlerintoplami = ucetambolunenler.reduce((toplam, currentValue) => {
  return toplam + currentValue;
}, 0);
...
const bosArray = [];
for (const property in tekraredensayilar) {
  if (tekraredensayilar[property] > 1) {
    bosArray.push(
      `${property} sayısı ${tekraredensayilar[property]} kere tekrar edilmistir`
    );
  }
}

console.log("tekraredensayilar", tekraredensayilar);
console.log("bosArray", bosArray);
